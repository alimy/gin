...
func TestValidateNoValidationValues(t *testing.T) {
	...
	assert.Nil(t, ValidateWith(test))
	assert.Nil(t, ValidateWith(&test))
	assert.Nil(t, ValidateWith(empty))
	assert.Nil(t, ValidateWith(&empty))
...
func TestValidatePrimitives(t *testing.T) {
	...
	assert.NoError(t, ValidateWith(obj))
	assert.NoError(t, ValidateWith(&obj))
	assert.Equal(t, Object{"foo": "bar", "bar": 1}, obj)

	obj2 := []Object{{"foo": "bar", "bar": 1}, {"foo": "bar", "bar": 1}}
	assert.NoError(t, ValidateWith(obj2))
	assert.NoError(t, ValidateWith(&obj2))

	nu := 10
	assert.NoError(t, ValidateWith(nu))
	assert.NoError(t, ValidateWith(&nu))
	assert.Equal(t, 10, nu)

	str := "value"
	assert.NoError(t, ValidateWith(str))
	assert.NoError(t, ValidateWith(&str))
...
// See https://github.com/alimy/gin/pull/1015.
func notOne(
